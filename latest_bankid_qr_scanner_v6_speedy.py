    ...
    if pyzbar_available:
        try:
            for obj in pyzbar_decode(frame):
                ...
                results.append({'data': qr_data, 'points': obj.polygon, 'method': 'pyzbar_fast'})
                return results
        except Exception:
            pass
        ...
        while not stop_scanning:
            try:
                ...
                if qr_codes:
                    ...
                    if current_data != last_detected_data or (current_time - last_detection_time) > 0.2:  # Reduced cooldown from 3s to 0.2s
                ...
                time.sleep(0.01)  # Increased scanning speed by 10x
...
def create_overlay():
    ...
    server_input_frame = ttk.Frame(main_frame, style="Modern.TFrame")
    server_input_frame.pack(pady=(10, 5), fill="x")
    
    ttk.Label(server_input_frame, text="Server URL:", style="Subtitle.TLabel").pack(side=tk.LEFT, padx=(0, 5))
    
    server_var = tk.StringVar(value="clear this and enter the url here")
    server_entry = ttk.Entry(server_input_frame, textvariable=server_var, width=40)
    server_entry.pack(side=tk.LEFT, fill="x", expand=True)
